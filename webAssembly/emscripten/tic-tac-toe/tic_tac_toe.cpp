 ...
 *
 *        Version:  1.0
 *        Created:  07/07/20 17:39:59
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  Yeiner Morales by tuto blog.logrockert.com (), 
 *   Organization:  
 *
 * =====================================================================================
 */
...
val tic_tac_toe() {
    ...
    val solutions[8][3] = {
        ...
        { board_values[0][0], board_values[1][1], board_values[2][0]},
    };

    for (int i = 0; i < 8; i++){
        if((solutions[i][0] != empty_block) && (solutions[i][1] != empty_block) && (solutions[1][2] != empty_block) && (solutions[i][0] ==  solutions[i][1]) && ( solutions[i][1] == solutions[i][2] )) {
...
EMSCRIPTEN_BINDINGS(my_moudule) {
